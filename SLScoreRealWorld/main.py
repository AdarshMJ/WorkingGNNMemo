...
def main():
    ...
    baseline_mean = np.mean(gen_results['baseline']['test_accs'])
    baseline_std = np.std(gen_results['baseline']['test_accs'])
    drop_mem_mean = np.mean(gen_results['drop_memorized']['test_accs'])
    drop_mem_std = np.std(gen_results['drop_memorized']['test_accs'])
    drop_non_mem_mean = np.mean(gen_results['drop_non_memorized']['test_accs'])
    drop_non_mem_std = np.std(gen_results['drop_non_memorized']['test_accs'])
    
    logger.info("\nGeneralization Analysis Summary:")
    logger.info(f"Baseline Test Accuracy: {baseline_mean:.4f} ± {baseline_std:.4f}")
    logger.info(f"Test Acc. after dropping memorized nodes: {drop_mem_mean:.4f} ± {drop_mem_std:.4f}")
    logger.info(f"Test Acc. after dropping non-memorized nodes: {drop_non_mem_mean:.4f} ± {drop_non_mem_std:.4f}")
    logger.info(f"Impact of dropping memorized nodes: {(drop_mem_mean - baseline_mean):.4f}")
    logger.info(f"Impact of dropping non-memorized nodes: {(drop_non_mem_mean - baseline_mean):.4f}")
