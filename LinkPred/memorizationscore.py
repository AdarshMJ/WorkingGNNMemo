...
def plot_edge_memorization_analysis(
    ...
    # Create additional zoomed views specifically for high memorization scores
    plt.figure(figsize=(15, 12))
    
    # First, create a table summarizing the high score edges
    plt.subplot(221)
    table_data = []
    ...
    # Bar chart for positive edges above threshold
    plt.subplot(223)
    ...
    for edge_type in edge_types_to_plot:
        if edge_type in edge_scores:
            edge_types.append(labels[edge_type])
    ...
    if edge_types:
        bars = plt.bar(edge_types, pos_values, color=[colors[et] for et in edge_types_to_plot if et in edge_scores])
    ...
    # Bar chart for negative edges above threshold
    plt.subplot(224)
    ...
    if edge_types:
        bars = plt.bar(edge_types, neg_values, color=[colors[et] for et in edge_types_to_plot if et in edge_scores])
        plt.ylabel('Percentage of Edges > 0.5 (%)')
    ...
    # Keep the scatter plot visualization for confidence comparison
