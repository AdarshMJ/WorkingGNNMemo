        ...
        if use_per_epoch_sampling:
            ...
            for category in edge_categories:
                ...
                while len(neg_edges_set) < num_neg_samples and attempts < max_attempts:
                    ...
                    for edge in neg_edge_index:
                        ...
                        if edge_tuple not in existing_edges_set and edge_tuple not in neg_edges_set:
                            # Ensure this negative edge is not a positive edge in any category
                            if edge_tuple not in all_pos_edges_set:
                ...
                if len(neg_edges) < num_neg_samples:
                    print(f"Warning: Could only sample {len(neg_edges)}/{num_neg_samples} valid negative edges for {category} after {attempts} attempts")
                
                if neg_edges:  # Only proceed if we have some valid edges
            ...
            # Combine all negative edges if we have any
            if all_neg_edges:
                ...
                if epoch == 0 or (epoch + 1) % 20 == 0:
                    for cat in edge_categories:
                        if cat in neg_edges_by_category:
                            ...
                            print(f"Epoch {epoch+1}: Using {size} valid {cat} negative edges")
                ...
                # Fallback to use only positive edges if no valid negatives found
                print(f"WARNING: No valid negative edges found for epoch {epoch+1}, training with positive edges only")
                current_train_edges = pos_edge_index
                current_edge_labels = torch.ones(pos_edge_index.size(1), dtype=torch.float, device=device)
                out = model.decode(z, current_train_edges).view(-1)
                # Use a custom loss that encourages high scores for positive edges
                loss = torch.mean((1 - out) ** 2)  # MSE loss pushing predictions toward 1
        else:
...
def train_model_single_seed(data_full, edges_dict, args, logger, model_type='f'):
    ...
    # Log whether we're using per-epoch negative sampling
    if hasattr(args, 'per_epoch_sampling') and args.per_epoch_sampling:
        ...
        logger.info(f"  - Using fixed negative sampling (same negative edges for all epochs)")
    ...
    # Train model with appropriate negative sampling approach based on args
    if hasattr(args, 'per_epoch_sampling') and args.per_epoch_sampling:
        ...
        logger.info(f"Training model {model_name} with fixed negative sampling...")
        model = train_link_prediction_model(
            model=model, 
            optimizer=optimizer, 
            data=train_data, 
