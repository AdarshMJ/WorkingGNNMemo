...
def plot_memorization_nli_comparison(results, save_path, k_hops):
    """Create enhanced visualization comparing NLI scores between memorized and non-memorized nodes"""
    ...
    # Calculate number of node types for subplot layout
    ...
    fig = plt.figure(figsize=(15, 4 * n_types))
    ...
    for idx, node_type in enumerate(node_types, 1):
        ...
        # Create box plots (more robust than violin plots)
    ...
    # Update overall title to include homophily information
    if homophily_ratio is not None:
        ...
        plt.suptitle(f'Node Label Informativeness Analysis - {homophily_text}\n' +
                    f'(k={k_hops}-hop neighborhood) Comparing Memorized (>0.5) vs Non-memorized (≤0.5) Nodes',
                    fontsize=14, y=1.02)
    else:
        plt.suptitle(f'Node Label Informativeness Analysis\n' +
                    f'(k={k_hops}-hop neighborhood) Comparing Memorized (>0.5) vs Non-memorized (≤0.5) Nodes',
                    fontsize=14, y=1.02)
    ...
    for node_type in node_types:
        if node_type in results:
            mem_scores = [item['nli_score'] for item in results[node_type]['memorized']]
            non_mem_scores = [item['nli_score'] for item in results[node_type]['non_memorized']]
