...
def forward_with_edge_weights(self, x, edge_index, edge_weights=None, return_node_emb=False, embedding_layer=None):
    """
    ...
    Args:
        ...
        edge_weights: Optional edge weights for message passing
        return_node_emb: Whether to return node embeddings along with the output
        embedding_layer: Which layer's embeddings to return (0-based index, None means last hidden layer)
    """
    embeddings = []
    
    # Process through all but the last convolution layer
    for i, conv in enumerate(self.convs[:-1]):
        x = conv(x, edge_index, edge_weight=edge_weights)
        x = F.relu(x)
        x = F.dropout(x, p=0.5, training=self.training)
        embeddings.append(x)  # Store embeddings after each hidden layer
